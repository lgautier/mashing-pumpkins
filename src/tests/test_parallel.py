...
def _make_sequence():
    return b''.join(
        random.choice((b'A', b'T', b'G', b'C')) for x in range(250)
    )
...
def test_sketch_initializer():
    ...
    def hashfun(input, width, hashbuffer):
        return None

    seed = 0
    cls = minhashsketch.MaxSketch
    mashingpumpkins.parallel.Sketch.initializer(
        cls, nsize, maxsize, hashfun, seed
    )
...
def test_sketch_map_sequences():
    ...
    sequence = _make_sequence()
    sequences = (
        sequence[beg:end] for beg, end in
        chunkpos_iter(nsize, len(sequence), 100)
    )
...
def test_sketchlist_initializer():
    ...
    mashingpumpkins.parallel.SketchList.initializer(
        clslist, [(nsize, maxsize, hashfun, seed)]
    )
    hasattr(mashingpumpkins.parallel, 'sketchlist_constructor')
    tpl = tuple(mashingpumpkins.parallel.sketchlist_constructor())
    assert len(tpl) == len(clslist)
    for elt, cls in zip(tpl, clslist):
    ...
    tpl = tuple(mashingpumpkins.parallel.sketchlist_constructor())
    assert len(tpl) == 2
    for elt, (nsize, maxsize, hashfun, seed) in zip(tpl, argslist):
...
def test_sketchlist_reduce_sketches():
    ...
    mhslist_ab = (
        mashingpumpkins.parallel.SketchList.reduce(mhslist_a, mhslist_b)
    )
